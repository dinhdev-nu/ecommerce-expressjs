...
const { BadRequestError, NotFoundError } = require("../response/error.respon");
const {
  product,
  clothing,
  furniture,
...
const {
  ...
  getProductDraftsByShop,
  getProductPublishedByShop,
  updateProductDraft,
  updateProductPublished,
  deleteProduct,
  update,
  getOneProductPublish,
  getOneProductDraft,
  deleteChildProduct,
  filterProduct,
...
class ProductFactory {
  ...
  static async filterProduct(query) {
    ...
    if (product_price)
      filter.product_price.$lte = product_price + product_price * 0.1;
  ...
  static async updateProduct({ product_id, payload, type }) {
    ...
    if (findProduct) {
      throw new BadRequestError(
        "Product is published! Pls Change to draft to update!"
      );
    }
    const classRef = ProductFactory.list_type[type];
    if (classRef) {
  ...
  static async deleteProduct(product_id) {
    ...
    if (!product) {
      throw new BadRequestError(
        "Product is published or not found! Pls Change to draft to delete!"
      );
    }
...
class Clothing extends ProductService {
  ...
  async deleteProduct(product_id) {
    ...
    const deleteProductChild = await deleteChildProduct({
      product_id,
      model: clothing,
    });
    return {
