...
const { BadRequestError } = require("../response/error.respon");
const {
  createInventory,
...
const updateInventoryForProduct = async ({
  ...
  try {
    ...
    await product.findByIdAndUpdate(inventory.inventory_productId, {
      product_quantity: quantity_new,
    });
...
const getInventoryForProduct = async ({ shop_id, product_id }) => {
  return await findInventory({ shop_id, product_id });
};

const deleteInventoryForProduct = async ({ shop_id, product_id }) => {
